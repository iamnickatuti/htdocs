...
// Output the JSON array
    ...
    $url2 = "http://localhost/production/api/conversion.php";
...
// Convert the associative array to a sequential array
    ...
    foreach ($data as $record) {
        ...
        // Check if the part number has been processed
        if (!in_array($partNumber, $partNumbers)) {
            ...
            for ($i = $startIndex; $i < count($keys); $i++) {
                ...
                break; // Break the loop after the first row for each part number
            }
        }
    }
