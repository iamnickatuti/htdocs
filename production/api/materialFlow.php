...
// Iterate over each element in $cageReceipts array
foreach ($cageData as $item2) {
    // Flag to check if a match is found for item2
    $matchFound = false;

    // Iterate over each element in $material array
    ...
    // If no match is found for item2, assign quantity as 0
    if (!$matchFound) {
        $item2['Quantity'] = 0;
        $mergedArray[] = $item2;
    }
