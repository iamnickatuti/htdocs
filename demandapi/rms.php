...
function substituteRawMaterial($conn, $rawMaterial, $depth = 1)
{
    ...
    if ($result) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Check if Sub Raw Material starts with WP and depth is less than or equal to 10
...
// Fetch the results
    ...
    while ($row = $result->fetch_assoc()) {
        $product = $row;
        // Check if Raw Material starts with WP and depth is less than or equal to 10
        if (strpos($row['Raw Material'], 'WP') === 0) {
            ...
            // Add the substituted raw materials to the product
            $product['Substituted Raw Materials'] = $subRawMaterials;
        }
